...
export default function CertificateSettingsPage() {
  ...
  const organizationId = session?.user?.organization?.id ?? null;

  const { data: organization } = useApi<OrganizationBranding | null>(
    organizationId ? `/organizations/${organizationId}` : "/organizations/none",
    null
  );
  ...
  useEffect(() => {
    ...
    setPrimaryColor(organization.primaryColor ?? DEFAULT_COLOR);
  }, [organization]);

  async function handleSave() {
    if (!organizationId) {
      setToast({ message: "No organization on this account — nothing to save to.", type: "error" });
      return;
    }
    setSaving(true);
    try {
      await apiMutate(`/organizations/${organizationId}/branding`, "PATCH", {
