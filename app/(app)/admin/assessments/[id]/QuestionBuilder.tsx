...
import { createQuestionAction, deleteQuestionAction, reorderQuestionAction } from "../actions";
...
const typeLabels: Record<AssessmentQuestion["type"], string> = {
  MULTIPLE_CHOICE: "Multiple choice",
  TRUE_FALSE: "True / False",
  SHORT_ANSWER: "Short answer",
};

export function QuestionBuilder({ assessmentId, questions }: { assessmentId: string; questions: AssessmentQuestion[] }) {
  const [newType, setNewType] = useState<AssessmentQuestion["type"]>("MULTIPLE_CHOICE");
  const [optionCount, setOptionCount] = useState(2);

  const boundCreateQuestion = createQuestionAction.bind(null, assessmentId);
        ...
        {questions.map((q, i) => (
          <div key={q.id} className="rounded-card border border-border bg-background px-4 py-3">
            <div className="flex items-start justify-between">
              <div>
                <span className="rounded-pill bg-blue/10 px-2 py-1 text-[13px] font-medium text-blue">
                  {typeLabels[q.type]} · {q.points} pt{q.points === 1 ? "" : "s"}
                </span>
              ...
              <div className="flex items-center gap-1">
                <form action={reorderQuestionAction.bind(null, assessmentId, q, questions[i - 1] ?? q)}>
                  <button
                    type="submit"
                    disabled={i === 0}
                ...
                <form action={deleteQuestionAction.bind(null, assessmentId, q.id)}>
        ...
        <form action={boundCreateQuestion} className="mt-4 space-y-3 rounded-card border border-dashed border-border p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <select
              ...
              value={newType}
              onChange={(e) => {
                setNewType(e.target.value as AssessmentQuestion["type"]);
                setOptionCount(2);
              }}
          ...
          {newType === "MULTIPLE_CHOICE" && (
            <div className="space-y-2">
              ...
              {optionCount < 6 && (
                <button type="button" onClick={() => setOptionCount((n) => n + 1)} className="text-[13px] font-medium text-blue hover:underline">
                  + Add option
                </button>
              )}
